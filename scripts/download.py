...
def load_law_sections(connection):
    '''
    '''
    DatRow = namedtuple('DatRow',[
        'id', 'law_code', 'section_num', 'op_statues', 'op_chapter',
        'op_section', 'effective_date', 'law_section_version_id', 'division',
        'title', 'part', 'chapter', 'article', 'history', 'content_xml',
        'active_flg', 'trans_uid', 'trans_update'])

    def dat_row_2_tuple(row):
        '''Convert a row in the law_section_tbl.dat file into a
        namedtuple.
        '''
        cells = row.split('\t')
        res = []
    ...
    sql = '''
        REPLACE INTO capublic.law_section_tbl
            (
                ID,
                LAW_CODE,
                SECTION_NUM,
    ...
    cursor = connection.cursor()
    with open('LAW_SECTION_TBL.dat') as f:
        for row in f:
            # The files are supposedly already in utf-8, but with
            # copious bogus characters.
            row = clean_text(row.decode('utf-8')).encode('utf-8')
            row = dat_row_2_tuple(row)
            with open(row.content_xml) as f:
                text = f.read().decode('utf-8')
                text = clean_text(text).encode('utf-8')
                row = row._replace(content_xml=text)
                cursor.execute(sql, tuple(row))
...
def load(folder, sql_name=partial(re.compile(r'\.dat$').sub, '.sql')):
    ...
    connection = MySQLdb.connect(
        user=MYSQL_USER, passwd=MYSQL_PASSWORD, host=MYSQL_HOST,
    ...
    for filename in filenames:
        ...
        elif sql_filename == 'law_section_tbl.sql':
            logger.info('inserting xml files (slow)')
            load_law_sections(connection)
        else:
            cursor = connection.cursor()
